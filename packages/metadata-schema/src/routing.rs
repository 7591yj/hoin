...
pub fn route_relative_destination(
    ...
    match model_name {
        "holo-hoin" => Ok(HoloHoinMeta::from_class_key(class_key).relative_destination(file_name)),
...
mod tests {
    ...
    fn routes_holo_hoin_predictions() {
        let path = route_relative_destination("holo-hoin", "amane_kanata", OsStr::new("input.png"))
            .unwrap();
    ...
    fn rejects_unknown_models() {
        let error = route_relative_destination("unknown-model", "example", OsStr::new("input.png"))
            .unwrap_err();
